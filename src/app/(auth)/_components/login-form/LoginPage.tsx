...
import { loginUser } from "@/core/server/actions/auth";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (state?.success) {
      ...
      router.push(`/dashboard?email=${encodeURIComponent(state.email)}`);
    }
  }, [state, router]);
    ...
    <main className="flex overflow-hidden flex-col bg-neutral-950">
      <section className="flex flex-col items-center px-20 pt-36 pb-64 w-full max-md:px-5 max-md:py-24 max-md:max-w-full">
        <div className="flex flex-col items-center mb-0 max-w-full w-[584px] max-md:mb-2.5">
          ...
          <form action={formAction} className="flex flex-col mt-14 w-full max-w-[584px] max-md:mt-10 max-md:max-w-full">
            <div className="flex overflow-hidden flex-col px-12 pt-12 pb-10 w-full rounded-2xl border border-solid bg-white bg-opacity-10 border-white border-opacity-10 max-md:px-5">
              <div className="flex flex-col w-full max-md:max-w-full">
                ...
                <RememberMeCheckbox />
                <button type="submit" className="self-stretch px-10 py-3 mt-6 w-full text-base font-medium bg-yellow-300 rounded-lg min-h-[48px] text-neutral-950 max-md:px-5 max-md:max-w-full">
                  Log In
                </button>
              </div>
